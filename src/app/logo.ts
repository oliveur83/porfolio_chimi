...
export class logo {
  ...
  init() {
    ...
    table.add(micro);
    window.addEventListener('click', toto);
  ...
  function toto(){
    ...
    // Animez la caméra pour qu'elle regarde vers la position cible
    const cameraTarget = new TWEEN.Tween(camera.position)
        .to({ x: targetPosition.x-6, y: targetPosition.y+6, z: targetPosition.z }, 1000) // Animez la caméra vers la position cible avec un décalage z pour qu'elle ne soit pas juste devant l'objet
